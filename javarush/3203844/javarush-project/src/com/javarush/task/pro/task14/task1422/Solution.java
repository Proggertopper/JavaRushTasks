...
public class Solution {
    ...
    public static void printDNA() {
            String format = "File name: %s, Method name: %s, Line number: %d\n";
            for(StackTraceElement[] element : getDNA()){
                for (int i = 1; i < 2; i++) {
                    System.out.printf(format , element[i].getFileName() , element[i].getMethodName() , element[i].getLineNumber());
                }
            }
    }
